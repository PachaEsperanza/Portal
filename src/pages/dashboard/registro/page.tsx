...
interface FormData {
  ...
  tipoFermentacion: string; diasFermentacion: string; diasSecado: string;
  tipoCafe: string; variedadCafe: string;
  primeraCosechaMes: string; primeraCosechaCantidad: string; primeraCosechaDuracion: string;
  segundaCosechaMes: string; segundaCosechaCantidad: string; segundaCosechaDuracion: string;
  terceraCosechaMes: string; terceraCosechaCantidad: string; terceraCosechaDuracion: string;
  kgPorHectarea: string; almacenamientoDetalle: string;
  parcelaCoords: ParcelaCoords | null;
  codigoParcela: string;
  edadPlantas: string; distanciamiento: string; numeroPlantas: string; pctProduccion: string;
  gradosBrix: string; fechaMuestreoBrix: string; practicasPostCosecha: string;
...
const SECTIONS: { id: SectionId; num: string; title: string; icon: string; required: (keyof FormData)[] }[] = [
  ...
  { id: "parcela",      num: "02", title: "Parcela y Georreferenciación", icon: "ri-map-pin-line",         required: ["nombreParcela","superficieTotal","areaCacao","fuenteHidrica","parcelaCoords","sistemaSombra","tipoFermentacion","diasFermentacion","diasSecado"] },
  { id: "cultivo",      num: "03", title: "Características del Cultivo",  icon: "ri-plant-line",           required: ["edadPlantas","distanciamiento","gradosBrix","tipoCafe","variedadCafe"] },
  { id: "produccion",   num: "04", title: "Producción y Cosecha",         icon: "ri-shopping-basket-line", required: ["estimadoAnual","tipoAcopio","kgPorHectarea","almacenamientoDetalle","primeraCosechaMes","primeraCosechaCantidad","primeraCosechaDuracion","segundaCosechaMes","segundaCosechaCantidad","segundaCosechaDuracion"] },
...
const init: FormData = {
  ...
  tipoFermentacion:"", diasFermentacion:"", diasSecado:"",
  tipoCafe:"", variedadCafe:"",
  primeraCosechaMes:"", primeraCosechaCantidad:"", primeraCosechaDuracion:"",
  segundaCosechaMes:"", segundaCosechaCantidad:"", segundaCosechaDuracion:"",
  terceraCosechaMes:"", terceraCosechaCantidad:"", terceraCosechaDuracion:"",
  kgPorHectarea:"", almacenamientoDetalle:"",
  parcelaCoords: null,
  codigoParcela:"",
  edadPlantas:"", distanciamiento:"", numeroPlantas:"", pctProduccion:"", gradosBrix:"", fechaMuestreoBrix:"",
  practicasPostCosecha:"", notasSensoriales:"",
...
export default function Registro() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const row = {
        ...
        telefono_familiar: finalData.telefonoFamiliar,
        nombre_familiar: finalData.nombreFamiliar,
        ...
        relacion_laboral: finalData.relacionLaboral,
        anios_experiencia: finalData.aniosExperiencia || null,
        tiene_cuenta: finalData.tieneCuenta,
        recibio_credito_agrario: finalData.recibioCreditoAgrario,
        tiene_deuda: finalData.tieneDeuda,
        antiguedad_comprador: finalData.antiguedadComprador || null,
        razon_comprador: finalData.razonComprador,
        disposicion: finalData.disposicion,
        tier: finalData.tier,
        ...
        vias_trocha: finalData.viasTrocha,
        vias_afirmada: finalData.viasAfirmada,
        vias_asfaltado: finalData.viasAsfaltado,
        vias_herradura: finalData.viasHerradura,
        vias_fluvial: finalData.viasFluvial,
        vias_pie: finalData.viasPie,
        distancia_via: finalData.distanciaVia || null,
        tiempo_flete: finalData.tiempoFlete || null,
        ...
        numero_plantas: finalData.numeroPlantas || null,
        pct_produccion: finalData.pctProduccion || null,
        grados_brix: finalData.gradosBrix || null,
        fecha_muestreo_brix: finalData.fechaMuestreoBrix || null,
        practicas_post_cosecha: finalData.practicasPostCosecha,
        notas_sensoriales: finalData.notasSensoriales,
        tipo_cafe: finalData.tipoCafe,
        variedad_cafe: finalData.variedadCafe,
        sistema_sombra: finalData.sistemaSombra,
        tipo_fermentacion: finalData.tipoFermentacion,
        dias_fermentacion: finalData.diasFermentacion || null,
        dias_secado: finalData.diasSecado || null,
        kg_por_hectarea: finalData.kgPorHectarea || null,
        almacenamiento_detalle: finalData.almacenamientoDetalle,
        primera_cosecha_mes: finalData.primeraCosechaMes,
        primera_cosecha_cantidad: finalData.primeraCosechaCantidad,
        primera_cosecha_duracion: finalData.primeraCosechaDuracion,
        segunda_cosecha_mes: finalData.segundaCosechaMes,
        segunda_cosecha_cantidad: finalData.segundaCosechaCantidad,
        segunda_cosecha_duracion: finalData.segundaCosechaDuracion,
        tercera_cosecha_mes: finalData.terceraCosechaMes,
        tercera_cosecha_cantidad: finalData.terceraCosechaCantidad,
        tercera_cosecha_duracion: finalData.terceraCosechaDuracion,
        estimado_anual: finalData.estimadoAnual || null,
        rendimiento_historico: finalData.rendimientoHistorico,
        comprador_actual: finalData.compradorActual,
        precio_kg: finalData.precioKg || null,
        almacenamiento: finalData.almacenamiento,
        tipo_acopio: finalData.tipoAcopio,
        transporte: finalData.transporte,
        costo_flete: finalData.costoFlete || null,
        meses_cosecha: finalData.mesesCosecha,
        cosecha_principal1_meses: finalData.cosechaPrincipal1Meses,
        cosecha_principal1_baba: finalData.cosechaPrincipal1Baba || null,
        cosecha_principal1_seco: finalData.cosechaPrincipal1Seco || null,
        cosecha_principal1_pago: finalData.cosechaPrincipal1Pago,
        cosecha_principal1_obs: finalData.cosechaPrincipal1Obs,
        cosecha_principal2_meses: finalData.cosechaPrincipal2Meses,
        cosecha_principal2_baba: finalData.cosechaPrincipal2Baba || null,
        cosecha_principal2_seco: finalData.cosechaPrincipal2Seco || null,
        cosecha_principal2_pago: finalData.cosechaPrincipal2Pago,
        cosecha_principal2_obs: finalData.cosechaPrincipal2Obs,
        cosecha_intermedia_meses: finalData.cosechaIntermediaMeses,
        cosecha_intermedia_baba: finalData.cosechaIntermediaBaba || null,
        cosecha_intermedia_seco: finalData.cosechaIntermediaSeco || null,
        cosecha_intermedia_pago: finalData.cosechaIntermediaPago,
        cosecha_intermedia_obs: finalData.cosechaIntermedaObs,
        total_anual_baba: finalData.totalAnualBaba || null,
        total_anual_seco: finalData.totalAnualSeco || null,
        total_anual_obs: finalData.totalAnualObs,
        plaga_principal: finalData.plagaPrincipal,
        pct_afectacion_principal: finalData.pctAfectacionPrincipal || null,
        plaga_secundaria: finalData.plagaSecundaria,
        pct_afectacion_secundaria: finalData.pctAfectacionSecundaria || null,
        meses_presion_plaga: finalData.mesesPresionPlaga,
        plaga_otra: finalData.plagaOtra,
        control_ecologico: finalData.controlEcologico,
        control_biologico: finalData.controlBiologico,
        control_quimico: finalData.controlQuimico,
        control_ninguno: finalData.controlNinguno,
        productos_control: finalData.productosControl,
        frecuencia_control: finalData.frecuenciaControl,
        estado_certificacion: finalData.estadoCertificacion,
        entidad_certificadora: finalData.entidadCertificadora,
        numero_certificado: finalData.numeroCertificado,
        fecha_vencimiento: finalData.fechaVencimiento || null,
        cert_nop: finalData.certNOP,
        cert_eu: finalData.certEU,
        cert_jas: finalData.certJAS,
        cert_fairtrade: finalData.certFairtrade,
        cert_rainforest: finalData.certRainforest,
        obs_certificacion: finalData.obsCertificacion,
        cap_municipalidad: finalData.capMunicipalidad,
        cap_midgri: finalData.capMIDGRI,
        cap_gore: finalData.capGORE,
        cap_devida: finalData.capDEVIDA,
        cap_ong: finalData.capONG,
        cap_empresa: finalData.capEmpresa,
        cap_cooperativa: finalData.capCooperativa,
        cap_ninguna: finalData.capNinguna,
        nombre_ong: finalData.nombreONG,
        tema_capacitacion: finalData.temaCapacitacion,
        tema_manejo: finalData.temaManejo,
        tema_fermentacion: finalData.temaFermentacion,
        tema_plagas: finalData.temaPlagas,
        tema_certificacion: finalData.temaCertificacion,
        tema_financiero: finalData.temaFinanciero,
        tema_buenas_practicas: finalData.temaBuenasPracticas,
        tema_mercados: finalData.temaMercados,
        tema_clima: finalData.temaClima,
        tema_gps: finalData.temaGPS,
        tema_asociatividad: finalData.temaAsociatividad,
        tema_nutricion: finalData.temaNutricion,
        comentarios_productor: finalData.comentariosProductor,
        confiabilidad: finalData.confiabilidad,
        calidad_grano: finalData.calidadGrano,
        potencial_estrategico: finalData.potencialEstrategico,
        riesgo_perdida: finalData.riesgoPerdida,
        accion_fidelizacion: finalData.accionFidelizacion,
        fecha_seguimiento: finalData.fechaSeguimiento || null,
        notas_confidenciales: finalData.notasConfidenciales,
        firma_productor: finalData.firmaProductor,
        huella_productor: finalData.huellaProductor,
        firma_acopiador: finalData.firmaAcopiador,
        fecha_lugar: finalData.fechaLugar,
        declaracion_jurada: finalData.declaracionJurada,
        raw_data: finalData,
      };
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      {/* PORTADA */}
      <div className={card}>
        <p className="font-bold text-amber-800 mb-1" style={{ fontFamily: "Calibri, 'Calibri', sans-serif", fontSize: "1.15rem", letterSpacing: "0.08em", textTransform: "uppercase" }}>Café de Especialidad</p>
      ...
      <div className={card + " space-y-5"}>
          ...
          <>
            ...
            <div className={`p-3 rounded-lg border-2 border-red-400 bg-red-50`}>
              ...
              <ul className="text-xs text-red-700 mt-1 space-y-0.5 list-disc pl-4">
                ...
                <li>Indica el tamaño total y cuánto está dedicado al café.</li>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div>
                <label className={lbl}>Hectáreas dedicadas al Café de Especialidad{reqStar}</label>
                ...
                {isErr("areaCacao") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Escribe las hectáreas de café</p>}
              </div>
              <div>
            ...
            <p className={sublbl}>2.1 — Sombra y procesamiento{reqStar}</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className={lbl}>¿Tiene sombra el cultivo?{reqStar}</label>
                <select className={I("sistemaSombra")} style={selectStyle} value={data.sistemaSombra} onChange={e => upd("sistemaSombra", e.target.value)}>
                  <option value="">👇 Selecciona una opción</option>
                  <option>Bajo sombra</option>
                  <option>No tiene sombra</option>
                  <option>Sombra parcial / intermitente</option>
                </select>
                {isErr("sistemaSombra") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Selecciona una opción</p>}
              </div>
              <div>
                <label className={lbl}>Tipo de fermentación{reqStar}</label>
                <select className={I("tipoFermentacion")} style={selectStyle} value={data.tipoFermentacion} onChange={e => upd("tipoFermentacion", e.target.value)}>
                  <option value="">👇 Selecciona una opción</option>
                  <option>Lavado (fermentación en agua)</option>
                  <option>Honey (miel)</option>
                  <option>Natural (café en fruta)</option>
                  <option>Fermentación anaeróbica</option>
                  <option>No fermenta / entrega directa</option>
                </select>
                {isErr("tipoFermentacion") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Selecciona una opción</p>}
              </div>
              <div>
                <label className={lbl}>Días de fermentación{reqStar}</label>
                <input className={I("diasFermentacion")} type="number" min="0" placeholder="Ej. 24 horas ≈ 1 día" value={data.diasFermentacion} onChange={e => upd("diasFermentacion", e.target.value)} />
                {isErr("diasFermentacion") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Escribe los días de fermentación</p>}
              </div>
              <div>
                <label className={lbl}>Días de secado{reqStar}</label>
                <input className={I("diasSecado")} type="number" min="0" placeholder="Ej. 10 días al sol" value={data.diasSecado} onChange={e => upd("diasSecado", e.target.value)} />
                {isErr("diasSecado") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Escribe los días de secado</p>}
              </div>
            </div>

            <p className={sublbl}>2.2 — Producción estimada al año</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className={lbl}>Producción estimada al año (quintales o kilos de Café)</label>
                <input className={inp} placeholder="Ej. 500 kilos / 5 quintales de Café seco" value={data.estimadoAnual} onChange={e => upd("estimadoAnual", e.target.value)} />
              </div>
              <div>
                <label className={lbl}>¿Cómo procesa y seca su café en la chacra?</label>
            ...
            <p className={sublbl}>2.3 — Vías de acceso a la chacra{reqStar}</p>
            ...
            <p className={sublbl}>2.4 — Abonos o productos que usa en su tierra (Normas de Europa)</p>
            <div className={`p-3 rounded-lg border-2 border-amber-400 bg-amber-50`}>
              <p className="text-xs text-amber-800 font-bold">⚠️ IMPORTANTE — Para exportar a Europa (Unión Europea) tu café debe ser orgánico. Indica exactamente qué productos usas en tu chacra.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
            ...
            <p className={sublbl}>2.5 — Ubicación satelital de la parcela <span className="text-red-600 normal-case">(requerido por la Unión Europea)</span></p>
            <ParcelaMapPicker
        ...
        {active === "cultivo" && (
          <>
            ...
            <div className={`p-3 rounded-lg border-2 border-red-400 bg-red-50`}>
              ...
              <ul className="text-xs text-red-700 mt-1 space-y-0.5 list-disc pl-4">
                <li>Indica la edad aproximada de tus plantas de café en años.</li>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                { label: "N.° estimado de plantas de café", k: "numeroPlantas" as const, ph: "Ej. 800 plantas" },
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div>
                <label className={lbl}>Tipo de café{reqStar}</label>
                <select className={I("tipoCafe")} style={selectStyle} value={data.tipoCafe} onChange={e => upd("tipoCafe", e.target.value)}>
                  <option value="">👇 Selecciona una opción</option>
                  <option>Arábica</option>
                  <option>Robusta</option>
                  <option>Mezcla / no lo sé</option>
                </select>
                {isErr("tipoCafe") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Selecciona el tipo de café</p>}
              </div>
              <div>
                <label className={lbl}>Variedad{reqStar}</label>
                <select className={I("variedadCafe")} style={selectStyle} value={data.variedadCafe} onChange={e => upd("variedadCafe", e.target.value)}>
                  <option value="">👇 Selecciona una opción</option>
                  ...
                  <option>Pache</option>
                  <option>Costa Rica 95</option>
                  <option>Catimor</option>
                  <option>Geisha</option>
                  <option>No lo sé / mezcla de variedades</option>
                </select>
                {isErr("variedadCafe") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Selecciona la variedad</p>}
              </div>
              <div>
              ...
              <div>
                <label className={lbl}>Notas sensoriales del café</label>
        ...
        {active === "produccion" && (
          <>
            ...
            <div className={`p-3 rounded-lg border-2 border-red-400 bg-red-50`}>
              <ul className="text-xs text-red-700 mt-1 space-y-0.5 list-disc pl-4">
                <li>Indica cuánto café produces al año en kilos (café pergamino o verde).</li>
                <li>El café generalmente tiene hasta 3 etapas de cosecha al año — completa cada una que aplique.</li>
                <li>Marca los meses en que cosechas tocando cada mes en el calendario.</li>
              </ul>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { label: "ESTIMADO ANUAL DE COSECHA — KG CAFÉ{reqStar}", k: "estimadoAnual" as const, ph: "kg café/año" },
                ...
                { label: "KG por hectárea{reqStar}", k: "kgPorHectarea" as const, ph: "Ej. 900 kg/ha" },
              ].map(f => (
                <div key={f.k}>
                  <label className={lbl}>{f.label.replace("{reqStar}","")}{f.label.includes("reqStar") && reqStar}</label>
                  <input className={isErr(f.k) ? inpErr : inp} placeholder={f.ph} value={String(data[f.k])} onChange={e => upd(f.k, e.target.value)} />
              ...
              <div>
                <label className={lbl}>Detalle del almacenamiento{reqStar}</label>
                <input className={I("almacenamientoDetalle")} placeholder="Ej. cuarto de madera, sacos de yute, silo, etc." value={data.almacenamientoDetalle} onChange={e => upd("almacenamientoDetalle", e.target.value)} />
                {isErr("almacenamientoDetalle") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Describe tu almacenamiento</p>}
              </div>
              <div>
                <label className={lbl}>Forma de transporte a la cooperativa</label>
            ...
            <p className={sublbl}>4.1 — Períodos de cosecha (el café tiene hasta 3 etapas al año){reqStar}</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="p-3 rounded-lg border border-stone-200 bg-stone-50">
                <p className="text-xs font-bold text-stone-700 mb-2">Primera cosecha{reqStar}</p>
                <label className={lbl}>¿En qué mes(es)?{reqStar}</label>
                <input className={I("primeraCosechaMes")} placeholder="Ej. Abril - Mayo" value={data.primeraCosechaMes} onChange={e => upd("primeraCosechaMes", e.target.value)} />
                {isErr("primeraCosechaMes") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Obligatorio</p>}
                <label className={lbl + " mt-2"}>¿Cuánto cosechas?{reqStar}</label>
                <input className={I("primeraCosechaCantidad")} placeholder="Ej. 300 kg" value={data.primeraCosechaCantidad} onChange={e => upd("primeraCosechaCantidad", e.target.value)} />
                {isErr("primeraCosechaCantidad") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Obligatorio</p>}
                <label className={lbl + " mt-2"}>¿Cuánto dura?{reqStar}</label>
                <input className={I("primeraCosechaDuracion")} placeholder="Ej. 3 semanas" value={data.primeraCosechaDuracion} onChange={e => upd("primeraCosechaDuracion", e.target.value)} />
                {isErr("primeraCosechaDuracion") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Obligatorio</p>}
              </div>
              <div className="p-3 rounded-lg border border-stone-200 bg-stone-50">
                <p className="text-xs font-bold text-stone-700 mb-2">Segunda cosecha{reqStar}</p>
                <label className={lbl}>¿En qué mes(es)?{reqStar}</label>
                <input className={I("segundaCosechaMes")} placeholder="Ej. Agosto - Setiembre" value={data.segundaCosechaMes} onChange={e => upd("segundaCosechaMes", e.target.value)} />
                {isErr("segundaCosechaMes") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Obligatorio</p>}
                <label className={lbl + " mt-2"}>¿Cuánto cosechas?{reqStar}</label>
                <input className={I("segundaCosechaCantidad")} placeholder="Ej. 500 kg" value={data.segundaCosechaCantidad} onChange={e => upd("segundaCosechaCantidad", e.target.value)} />
                {isErr("segundaCosechaCantidad") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Obligatorio</p>}
                <label className={lbl + " mt-2"}>¿Cuánto dura?{reqStar}</label>
                <input className={I("segundaCosechaDuracion")} placeholder="Ej. 1 mes" value={data.segundaCosechaDuracion} onChange={e => upd("segundaCosechaDuracion", e.target.value)} />
                {isErr("segundaCosechaDuracion") && <p className="text-xs text-red-600 mt-1 font-bold">⚠️ Obligatorio</p>}
              </div>
              <div className="p-3 rounded-lg border border-stone-200 bg-stone-50">
                <p className="text-xs font-bold text-stone-700 mb-2">Tercera cosecha <span className="text-stone-400 font-normal normal-case">(si aplica)</span></p>
                <label className={lbl}>¿En qué mes(es)?</label>
                <input className={inp} placeholder="Ej. Noviembre" value={data.terceraCosechaMes} onChange={e => upd("terceraCosechaMes", e.target.value)} />
                <label className={lbl + " mt-2"}>¿Cuánto cosechas?</label>
                <input className={inp} placeholder="Ej. 150 kg" value={data.terceraCosechaCantidad} onChange={e => upd("terceraCosechaCantidad", e.target.value)} />
                <label className={lbl + " mt-2"}>¿Cuánto dura?</label>
                <input className={inp} placeholder="Ej. 2 semanas" value={data.terceraCosechaDuracion} onChange={e => upd("terceraCosechaDuracion", e.target.value)} />
              </div>
            </div>
        ...
        {active === "sanidad" && (
          <>
            ...
            <div className={`p-3 rounded-lg border-2 border-red-400 bg-red-50`}>
              ...
              <ul className="text-xs text-red-700 mt-1 space-y-0.5 list-disc pl-4">
                <li>Indica cuál es la enfermedad que más daña tu café.</li>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                ...
                <select className={I("plagaPrincipal")} style={selectStyle} value={data.plagaPrincipal} onChange={e => upd("plagaPrincipal", e.target.value)}>
                  <option value="">👇 Selecciona una opción</option>
                  <option>Roya del café (Hemileia vastatrix)</option>
                  <option>Broca del café (Hypothenemus hampei)</option>
                  <option>Ojo de gallo (Mycena citricolor)</option>
                  <option>Antracnosis (mancha de manteca)</option>
                  <option>Cercospora (mancha de hierro)</option>
                  <option>Mal de hilachas</option>
        ...
        {active === "certificacion" && (
          <>
            ...
            <div className={`p-3 rounded-lg border-2 border-blue-400 bg-blue-50`}>
              <p className="text-xs text-blue-800 font-bold">ℹ️ La certificación orgánica nos permite exportar el café a Europa con un precio mucho mejor. Si no tienes certificación aún, ¡no te preocupes! Nosotros te ayudamos a conseguirla.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
              ...
              <div>
                ...
                <input className={inp} placeholder="Ej. manejo del café, fermentación..." value={data.temaCapacitacion} onChange={e => upd("temaCapacitacion", e.target.value)} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <Chk checked={data.temaManejo} onChange={v => upd("temaManejo", v)} label="Manejo del café (poda, fertilización, sombra)" />
              ...
              <Chk checked={data.temaMercados} onChange={v => upd("temaMercados", v)} label="Cómo vender mejor mi café" />
              ...
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <div>
                  ...
                  <p className="text-xs text-stone-600 leading-relaxed">
                    Al firmar esta ficha, el productor y el acopiador declaran bajo juramento que los datos son verídicos y han sido obtenidos con consentimiento informado. La información es reservada y su uso está restringido al programa de trazabilidad de café de especialidad.
                  </p>
                </div>
              </label>
